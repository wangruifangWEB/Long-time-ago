...
$(function(){
	...
	$('.radio_btn li:nth-child(2)').click(function(){
		
		if($('.radio_btn li:nth-child(2)').hasClass('isPlay')){
			$('.radio_btn li:nth-child(2) img').attr('src','../../images/human_market/radio_play.png');
			$('.radio_btn li:nth-child(2)').removeClass('isPlay');
			vid.pause();
		}else{
			$('.radio_btn li:nth-child(2) img').attr('src','../../images/human_market/radio_parse.png');
			$('.radio_btn li:nth-child(2)').addClass('isPlay');
			vid.play();
		}
	})
	function yininInfo(){
		// 全部时长
		var duration=vid.duration.toString().split('.')[0];
		...
		// 当前播放时间
		var currentTime=vid.currentTime.toString().split('.')[0];
		var currentTimeFirst=parseInt(currentTime/60);
		var currentTimeSecond=parseInt(currentTime%60);
		if(currentTimeSecond<10){
			currentTimeSecond='0'+currentTimeSecond;
		}
		$('.radio_info_currentTime').html(currentTimeFirst+':'+currentTimeSecond);
	}
	vid.ontimeupdate = function(){
		...
		$('.radio_show li:nth-child(1)').css({'width':scale+'%'});
		$('.radio_show li:nth-child(2)').css({'width':100-scale+'%'});
		$('.radio_play').css({'left':(scale-2)+'%'});
	};
})
